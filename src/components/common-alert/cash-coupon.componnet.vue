    ...
    <div class="cash-coupon">
      ...
      <div class="cash-coupon__content">
        <ul class="coupon-list">
          <li class="coupon dr-flex" v-for="(v,index) of allItem" :key="index" @click="selectWhich(index)">
            ...
            <div class="coupon-info flex-1">
              <div class="coupon-info__name">{{v.name}}</div>
              <!--<div class="coupon-info__expire">{{v.expire}}小时候过期</div>-->
              <div class="coupon-info__expire">{{v.startTime}} - {{v.endTime}}</div>
            </div>
            <div class="coupon-value">{{v.prize}}元</div>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    close() {
      this.show = false;
      this.$emit('cash-coupon-close');
    },
    btnAction() {
      this.show = false;
      this.$emit('cash-coupon-btn-action');
    },
    selectWhich(index) {
      this.selected = index;
    },
...
.coupon-select__img--active {
  background-image: url('/static/images/components/alert/checked.png');
