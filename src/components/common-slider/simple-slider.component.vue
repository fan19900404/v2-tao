...
<template>
  <div :style="sliderStyle" class='slider-container' :class='basicdata.containerClass'>
    <div class="slider-wrapper" :style="styleobj" :class="basicdata.animation" @touchmove="swipeMove" @touchstart="swipeStart" @touchend="swipeEnd" @mousedown="swipeStart" @mouseup="swipeEnd" @mousemove="swipeMove">
      <!-- 正常滚动 -->
      <template v-if="!sliderinit.loop">
        <template v-for="(item,index) in pages">
          <div class="slider-item" :style="item.style" :key="index">
            <a v-if="item.link" class="slider-item-link" :href="item.link">
              <img class="slider-item-img" :src="item.imgUrl">
              <span class="slider-title">{{ item.name }}</span>
            </a>
            <img v-if="!item.link" class="slider-item-img" :src="item.imgUrl">
          </div>
        </template>
      </template>
      <!-- 无缝滚动 -->
      <template v-if="sliderinit.loop">
        <div class="slider-item" :style="pages[pages.length-1].style">
          <a v-if="pages[pages.length-1].link" class="slider-item-link" @click="link(pages[pages.length-1].link)">
            <img class="slider-item-img" :src="pages[pages.length-1].imgUrl">
          </a>
          <img v-if="!pages[pages.length-1].link" class="slider-item-img" :src="pages[pages.length-1].imgUrl">
        </div>
        <template v-for="(item,index) in pages">
          <div class="slider-item" :style="item.style" :key="index">
            <a v-if="item.link" class="slider-item-link" @click="link(item.link)">
              <img class="slider-item-img" :src="item.imgUrl">
            </a>
            <img v-if="!item.link" class="slider-item-img" :src="item.imgUrl">
          </div>
        ...
        <div class="slider-item" :style="pages[0].style">
          <a v-if="pages[0].link" class="slider-item-link" @click="link(pages[0].link)">
            <img class="slider-item-img" :src="pages[0].imgUrl">
          </a>
          <img v-if="!pages[0].link" class="slider-item-img" :src="pages[0].imgUrl">
        </div>
      </template>
    </div>
  </div>
...
<script>
export default {
  props: ['sliderinit', 'pages', 'sliderStyle'],
  data() {
    return {
      basicdata: {
        poswidth: '0',
  ...
  computed: {
    ...
    currentWidth: {
      get() {
        ...
        for (const item in $sliderChildren) {
          if (item <= lastPage) {
