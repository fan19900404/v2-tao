<template>
  <div>
    <h2>!!!用到的过滤器函数需要额外引入</h2>
    <h3>游戏中心、游戏详情页-相关游戏</h3>
    <ul class="list">
      <router-link tag="li" :to="{name: 'tabbar'}" class="list-item" v-for="(item, index) in list" :key="index">
        <tmListItem :thumb="item.gameIcon" :title="item.name" :sub-title="item.category" :extra="item.abstract" :is-last="index == list.length-1" :border-type="1">
          <tmButton slot="right" text="进入" :has-action="true" @click="tt"></tmButton>
    ...
    <ul class="list">
      <li class="list-item" v-for="(item, index) in myGameList" :key="index">
        <tmListItem :thumb="item.imgUrl" :title="item.gameName" :sub-title="formatDateToTimeago(item.cDate * 1000)" :is-last="index == myGameList.length-1">
          <tmButton slot="right" text="打开" :has-action="true" @click="tt"></tmButton>
        </tmListItem>
      </li>
    </ul>
    <br>
    ...
    <ul class="list">
      <li class="list-item" v-for="(item, index) in rankList" :key="index">
        <tmListItem :index="index+1" :thumb="item.gameIcon" :title="item.name" :sub-title="item.category" :extra="item.abstract" :is-last="index == rankList.length-1">
          <tmButton slot="right" text="进入" :has-action="true" @click="tt"></tmButton>
          <span slot="right" class="item__desc"><em>{{item.viewNumber}}</em>人在玩</span>
        </tmListItem>
      </li>
    ...
    <tmListItem :thumb="hotTask.picPath" :title="hotTask.title" :sub-title="hotTask.content" :is-last="true">
      <tmButton slot="right" text="去完成" :has-action="true" @click="tt" v-if="hotTask.isComplete==1"></tmButton>
      <tmButton slot="right" text="领取" :has-action="true" @click="tt" v-if="hotTask.isComplete==2"></tmButton>
      <tmButton slot="right" text="已领取" :has-action="true" :disabled="true" v-if="hotTask.isComplete==3"></tmButton>
      <span slot="right" class="item__desc">{{hotTask.reward}}</span>
    </tmListItem>
    <br>
    ...
    <ul class="list">
      <li class="list-item" v-for="(item, index) in unOpenList" :key="index">
        <tmListItem :thumb="item.gameIcon" :title="item.name" :sub-title="item.category  + ' | ' + item.serviceName" :extra="dateFormat(item.mDate*1000, 'yyyy-MM-dd hh:mm:ss')" :is-last="index == unOpenList.length-1" :border-type="1">
          <tmButton slot="right" text="进入" :disabled="true" @click="tt"></tmButton>
        </tmListItem>
      </li>
    </ul>
    <br>
    ...
    <ul class="list">
      <li class="list-item" v-for="(item, index) in packs" :key="index">
        <tmListItem :thumb="item.image" :title="item.gameName" :sub-title="item.name" :extra="item.typeName" :is-last="index == packs.length-1" :border-type="1">
    ...
    <ul class="list">
      <li class="list-item" v-for="(item, index) in hotGifts" :key="index">
        <tmListItem :thumb="item.image" :title="item.gameName" :sub-title="item.name" :extra="item.content | removeHtmlTag" :is-last="index == hotGifts.length-1" :border-type="1">
          <TmProgress slot="right" :progress="(item.totalNumber-item.usedNumber==0)?0:item.usedNumber/item.totalNumber" :disabled="(item.uID||item.totalNumber-item.usedNumber==0)?true:false" :text="item.uID == 0&&item.taoDou !=0 ? '兑' :item.uID == 0?'领':'已领'"></TmProgress>
        </tmListItem>
      </li>
    </ul>
    ...
    <ul class="list">
      <li class="list-item" v-for="(item, index) in myGiftList" :key="index">
        <tmListItem :thumb="item.image" :title="item.name" :sub-title="item.name" :extra="'兑换码:'+item.no" :is-last="index == myGiftList.length-1" :border-type="1">
          <tmButton slot="right" text="查看" @click="tt"></tmButton>
        </tmListItem>
      </li>
    </ul>
    <br>
    ...
    <ul class="list">
      <li class="list-item" v-for="(item, index) in platformGift" :key="index">
        <tmListItem :thumb="item.image" :title="item.title" :sub-title="'兑换码:'+item.code" :extra="item.desc" :is-last="index == platformGift.length-1">
          <tmButton slot="right" text="已使用" :disabled="true" v-if="item.status==3"></tmButton>
          <tmButton slot="right" text="使用" :has-action="true" @click="tt" v-else></tmButton>
        </tmListItem>
      </li>
    </ul>
  </div>
