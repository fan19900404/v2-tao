...
<template>
	<app-header :title="title" :type="type" :personal="1">

	</app-header>
	<section class="userinfo-area">
		<section class="user-info-box flex-ld">
			<img class="avator" :src="userInfo.avatar" alt="头像" v-link="'/personal/userinfo'">
			<div class="name-id flex-col-end">
				<span class="user-name">{{userInfo.username}}</span>
				<span class="user-id">ID:{{userInfo.userId}}</span>
			</div>
			<!-- <span class="check-jf" v-if="userInfo.isShowSoreExchange == 1" v-link="'/welfare/integral-exchange'">积分兑换</span>
      <span v-else class="temp">&nbsp;</span> -->
      <span class="temp">&nbsp;</span>
      <div class="flex quick-link">
        <i class="flex-1 icon icon-cus cus" v-link="'/personal/service'"></i>
        <i class="flex-1 icon icon-settings cus" v-link="'/personal/userinfo'"></i>
      </div>
		</section>
		<section class="money-box flex">
			<div class="each flex-1 flex-col-end">
				<span class="m-name" v-link="'welfare/taobi'">淘币:<span class="m-count">{{userInfo.taoBi}}</span></span>
				<a class="btn" v-link="'/personal/recharge'">充值</a>
			</div>
			<span class="sep"></span>
			<div class="each flex-1 flex-col-end">
				<span class="m-name" v-link="'welfare/taodou'">淘豆:<span class="m-count">{{userInfo.taoDou}}</span></span>
				<a href="#" class="btn" v-link="'/welfare/taodou-dh'">兑换夺宝券</a>
			</div>
		</section>
	</section>
    <!-- 填充 -->
    ...
    <section id="J_top_fixed" class="tab bg-fff flex" :class="{'header-active': !showHeader}">
    	<a class="flex-1" :class="{'active':tabIndex == 0 || tabIndex == 1 || tabIndex == 2}" @click="selectType()">
    		<select v-on:change="tabChange(selected)" class="select-section" v-model="selected">
    			<option value="0" selected>全部参与</option>
    			<option value="1">进行中</option>
    			<option value="2">已揭晓</option>
    		</select>
    		<i class="one-mall-iconfont icon-keyboard_arrow_down arrow-drop"></i></span>
    	</a>
    	<a class="tab-a flex-1" :class="{'active':tabIndex == 3}"  @click="tabChange(3)">幸运记录</a>
    	<a class="tab-a flex-1" :class="{'active':tabIndex == 4}"  @click="tabChange(4)">晒单记录</a>
    </section>
    ...
    <!-- 客服 -->
    <a v-link="'/personal/service'" class="back-home-kefu icon icon-cus"></a>
    <!-- 客服 -->
    		...
    		getRecordList(url,first){
    			...
    			if(!_this.recordLoadAll){
    				Vue.OneMallHttp(this).GET(params,url).then(function(data){
    					...
    					//是否加载完全
    					_this.recordLoadAll = (data.result.list.isLast == 1);
    					_this.recordPage++;
    					mallUtils.funs.scroll(_this.bindScrollEvent);
    					//如果是第一次进来将用户信息赋值
    					if(first){_this.userInfo = data.result.info}
    				})
    			}
    		},
    		//绑定上拉滚动加载更多数据
