  ...
  <div>
    ...
    <div class="container" v-if="timerGift.length > 0">
     ...
     <div class="present" v-if="hotGift.inited && hotGift.list.length > 0">
        ...
        <ul class="present-list clearfix">
          <li class="item fl" v-for="(item,i) in hotGift.list.slice(0,4)" @click="link({name: 'TaoYouWelfareShopWelfareDetails', params:{id: item.id}})" :key="i">
            <div class="item__wrapper">
              <img class="item__img" :src="item.photograph">
              <div class="item__name overflow-ellipsis">{{item.name}}</div>
              <div class="item__exchange">
                <div class="item__exchange-price"><em>{{item.taoDou}}淘豆</em></div>
                <div class="item__exchange-btn" v-if="item.uID==0 && item.taoDou==0" @click.stop="button(item)">领取</div>
                <div class="item__exchange-btn" v-if="item.uID==0 && item.taoDou>0" @click.stop="button(item)">兑换</div>
                <div class="item__exchange-btn item__exchange-btn--disabled" v-if="item.uID>0">已领</div>
              </div>
     ...
     <div class="header" v-link="{name:'TaoYouWelfareShopWelfareExplain'}">
        <span>优惠券</span>
        <a class="use_explain JS-statistics-intro"><span class="icon icon-usage"></span><span class="text">使用说明</span></a>
      </div>
        ...
        <ul class="JS-statistics-list" v-infinite-scroll="getTicketList()" infinite-scroll-distance="10" infinite-scroll-disabled="ticket.loading || ticket.allLoaded" infinite-scroll-immediate-check="false">
          <li v-for="(ticket,index) in ticket.list" class="ticket" :key="index">
            <div class="ticket__left overflow-ellipsis">
              ...
              <p class="ticket__reqire">满<span>{{ticket.remark}}</span>元可使用</p>
              <p class="ticket__date">有效期至<span>{{formatDate (ticket.endTime,'YYYY-MM-DD HH:MM')}}</span></p>
              <p class="ticket__remark"><span>{{ticket.prize | moneyWidthComma}}淘豆</span></p>
            </div>
     ...
     <dialog-component-taodou :show="showDialogTaodou" :has-slot="true" left-btn-text="赚淘豆" right-btn-text="继续兑换" @two-btns-dialog-component-left="leftEvent" @two-btns-dialog-component-right="rightEvent">
        <div class="dialog-content" slot>
          <div class="result">兑换失败</div>
          <div class="reason">{{reason}}</div>
        </div>
      </dialog-component-taodou>
    <!--两个按钮的弹窗-->
    <!-- <two-button-dialog :show-title="showTitle" :content="content" :show="isShow" :has-slot="hasSlot" :left-btn-text="leftBtnText" :right-btn-text="rightBtnText" :show-close-btn="showCloseBtn" @two-btns-dialog-component-left="closeDialog" @two-btns-dialog-component-right="confirmOk" :btn-text="btnText">
        <div slot="addInner" v-if="hasSlot">
          <p class="p1">兑换此礼包需要消耗<span>{{count}}</span>淘豆</p>
          <p class="p2">是否兑换？</p>
          <p class="p3" v-link="{name:'TaoYouTaoDouExplain'}">什么是淘豆？</p>
        </div>
      </two-button-dialog>
...
// '../../../components/tao-you/dialog/two-buttons-simple-dialog.component.vue';
// import DialogComponentSurplus from
// '../../../components/tao-you/dialog/one-button-simple-dialog.component.vue';
// import DialogComponentTaodou from
// '../../../components/tao-you/dialog/two-buttons-simple-dialog.component.vue';
// import ProgressComponent from
// '../../../components/tao-you/svg-progress/svg-progress.component.vue';
...
// import TwoButtonDialog from
// '../../../components/tao-you/dialog/two-buttons-link-dialog.component';
// import Toast from '../../../components/tao-you/toast/toast.component.vue';
...
//   // 倒计时
// import Timer from '../../../components/tao-you/timer/date-timer.component.vue';
...
export default {
  ...
  //           this.updateUserInfo({
  // taoDou: parseInt(this.user.taoDou) - parseInt(_this.currentItem.taodou) });
