<template>
  <div>
    ...
    <div class="container">
      ...
      <div class="container__name">{{pd.name}}</div>
      <div class="container__desc">
        <span class="container__count">
          <i class="container__count--img"></i>{{count}} 淘豆</span>
        <span class="container__much">剩余 {{parseInt(pd.totalNumber)-parseInt(pd.usedNumber)}}</span>
      </div>
...
.container {
  ...
  &__name {
    ...
    font-size: .16rem;
    color: #333;
    padding: 0.128rem 0.15rem 0.1024rem 0.15rem;
  }
  &__count {
    font-size: .13rem;
    color: #ff4a4a;
